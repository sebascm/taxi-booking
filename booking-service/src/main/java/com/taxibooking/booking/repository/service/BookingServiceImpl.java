...
public class BookingServiceImpl implements BookingService {
  ...
  @Autowired private TaxiRepository taxiRepository;
   ...
   * Book a taxi.
   *
   * @param bookingDto booking data transfer object. Requires: - username username of passenger. -
   *     numberPassengers number of passengers. - pickup location - destination location
   * @throws AccountAuthenticationFailed if account authentication fails.
   * @throws RouteNotFoundException route not found.
   * @throws InvalidGoogleApiResponseException invalid response from Google (Invalid JSON as there
   *     API has changed).
    ...
    try {
      ...
      Route route =
          this.googleDistanceMatrixService.getRouteInfo(
              bookingDto.getStartLocation(), bookingDto.getEndLocation());
   ...
   * If passenger has multiple roles combine collections.
   *
  ...
  public Booking checkActiveBooking(String username) {
    ...
    List<Booking> bookings =
        this.bookingRepository.findByPassenger(this.accountRepository.findOne(username));
