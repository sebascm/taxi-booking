...
public class GoogleDistanceMatrixServiceImpl implements GoogleDistanceMatrixService {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(GoogleDistanceMatrixServiceImpl.class.getName());
   ...
   *
   * <p>Example query: http://maps.googleapis.com/maps/api/geocode/json?address=%22po5%201pl%22
   *
  ...
  /**
   * Get route info (distance, route, travel time, start and end textual address) using start and
   * end location.
   *
  ...
  /**
   * Get routes from valid Google Directions API response. Pre-condition: response contain a valid
   * route e.g. API response status is OK. Example
   *
   * <p>Example query: https://maps.googleapis.com/maps/api/directions/json?origin
      ...
      for (int i = 0; i < routes.length(); i++) {
        ...
        for (int j = 0; j < legs.length(); j++) {
          ...
          for (int k = 0; k < steps.length(); k++) {
            String polyline =
                (String) ((JSONObject) ((JSONObject) steps.get(k)).get("polyline")).get("points");
   ...
   */
  @Override
  public long estimateTravelTime(Location startLocation, Location endLocation)
      throws InvalidGoogleApiResponseException {
    ...
    try {
      ...
      if (this.validateJsonResponse(json)) {
        ...
        // time in seconds
        estimatedTravelTime =
            (long) legs.getJSONObject(0).getJSONObject("duration").getDouble("value");
      }
